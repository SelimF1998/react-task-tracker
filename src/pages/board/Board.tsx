...
const Board = () => {
  ...
  setInProgressTasks(prev => prev.filter(task => task.id !== taskToDelete.id));
  setOnApprovalTasks(prev => prev.filter(task => task.id !== taskToDelete.id));
  ...
  if (editClicked) {
    ...
    const removeOldTask = (tasks: Task[]) => tasks.filter(task => task.id !== updatedTask.id);
  ...
  return (
    <div className='board' >
      ...
      <AddTaskFormDialog editClicked={editClicked} editingTask={editingTask} isOpen={isAddTaskDialogOpen} onClose={closeDialog} onSubmit={handleTaskSubmit} />
      {/* <TaskCard /> */}
      <div className='board__tasks-container' >
        <div className='board__tasks-container__body' >
          <TasksContainer onEdit={handleEditTask} onDelete={handleDeleteTask} tasks={todoTasks} label="Todo" color="#FDF8EA" />
        </div>
        <div className='board__tasks-container__body' >
          <TasksContainer onEdit={handleEditTask} onDelete={handleDeleteTask} tasks={inProgressTasks} label="In Progress" color="#F0F0FF" />
        </div>
        <div className='board__tasks-container__body' >
          <TasksContainer onEdit={handleEditTask} onDelete={handleDeleteTask} tasks={onApprovalTasks} label="On Approval" color="#E6FEEB" />
        </div>
        <div className='board__tasks-container__body' >
          <TasksContainer onEdit={handleEditTask} onDelete={handleDeleteTask} tasks={doneTasks} label="Done" color="#FBEEF6" />
        </div>
      </div>
    </div>
  )
